...
2. Service account JSON path set via `GDRIVE_SERVICE_ACCOUNT_PATH` (gitignored)
...
```bash
python .cursor/skills/gdrive-specs-sync/scripts/publish_spec.py specs/features/<name>.md
```
...
```bash
python .cursor/skills/gdrive-specs-sync/scripts/fetch_comments.py <gdrive_doc_id>
```
...
```bash
python .cursor/skills/gdrive-specs-sync/scripts/publish_spec.py specs/features/<name>.md --update
```
...
- **publish_spec.py**: Upload/update spec to Google Drive
- **fetch_comments.py**: Retrieve and parse comments from Google Docs
